...
hitsound = mixer.Sound("sounds/hit.wav")
losesound = mixer.Sound("sounds/lose.wav")
...
class Ball(GameSprite):
    ...
    def update(self):
        ...
        if self.rect.x <= 0:
            lose = transform.scale(image.load("images/1lose.png") , (700, 500))
            window.blit(lose, (0, 0))
            losesound.play()
            
            run = False
        if self.rect.x >= 640:
            lose = transform.scale(image.load("images/2lose.png") , (700, 500))
            window.blit(lose, (0, 0))
            losesound.play()
